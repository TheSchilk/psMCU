...
def parse_args(args):
    ...
    parser.add_argument('-s', required=False, action="store_true", help="Do not add program startup section.")
    ...
    if parsed_args.input_file.endswith('.psOBJ'):
        setattr(parsed_args, 'input_type', 'psOBJ')
    elif parsed_args.input_file.endswith('.psASM'):
        setattr(parsed_args, 'input_type', 'psASM')
    else:
