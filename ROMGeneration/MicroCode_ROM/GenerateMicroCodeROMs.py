...
with open(sys.argv[1], 'r', newline='') as file:
    ...
    print("Parsing " + str(rom_count) + " ROMs each with " + str(length) + " entries.")
        ...
        for rom_index in range(0, rom_count):
            ...
            for val in entries:
                if val != '0' and val != '1':
                    ...
                    print(range(col_start + 8 * rom_index, col_start + 8 * rom_index + 8))
                    sys.exit()
            ...
            ROMs[rom_index].append(int(val[::-1], 2).to_bytes(1, byteorder='big', signed=False))
